  ...
  if (!plan) {
    return (
      <SafeAreaView style={styles.safe}>
        <Text style={{ color: Colors.muted, textAlign: 'center', marginTop: 40 }}>Plan not found.</Text>
      </SafeAreaView>
    );
  }
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top','left','right']}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => <ExerciseCard exercise={item} accentColor={plan.color} />}
        contentContainerStyle={styles.list}
        showsVerticalScrollIndicator={false}
        ListFooterComponent={
          <View style={styles.footer}>
            <View style={[styles.footerCard, { borderColor: plan.color + '30' }]}>
              <Text style={[styles.footerTitle, { color: plan.color }]}>💡  How to use this plan</Text>
...
const styles = StyleSheet.create({
  ...
  planSubtitle: { color: Colors.muted, fontSize: 12, marginTop: 2 },
  planCount: { color: Colors.dim, fontSize: 11, marginTop: 4 },
  list: { paddingBottom: 20 },
  footer: { padding: 16, paddingTop: 8 },
  footerCard: { backgroundColor: Colors.surface2, borderRadius: 12, borderWidth: 1, padding: 14 },
  footerTitle: { fontSize: 13, fontWeight: '700', marginBottom: 8 },
